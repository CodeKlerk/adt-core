...
class UpdatePatientEvent extends Event
{
    ...
    public function handle(){
        ...
        if(array_key_exists('who_stage_id', $this->patient)){
        ...
        if(array_key_exists('drug_name', $this->patient)){
            $patient_drug = PatientDrugOther::where('patient_id', $this->id);
            $patient_drug->update([
                'drug_name' => $this->patient['drug_name']
            ]);
        }
        if(array_key_exists('allergy_name', $this->patient)){
            $patient_other_allergy = PatientDrugAllergyOther::where('patient_id', $this->id);
            $patient_other_allergy->update([
                'allergy_name' => $this->patient['allergy_name']
            ]);
        }
        if(array_key_exists('illnesses', $this->patient)){}
        if(array_key_exists('other_illnesses', $this->patient)){
            $patient_other_illness = PatientIllnessOther::where('patient_id', $this->id);
            $patient_other_illness->update([
                'other_illness' => $this->patient['other_illnesses']
            ]);
        }
