...
class TestController extends Controller
{
    ...
    public function get_test()
    {
        ...
        $response = DB::table('tbl_visit')
                        ...
                        ->get();
        // // $response = DB::select('SELECT tv.patient_id as patient,    tsi.drug_id as drug, DATEDIFF(now(),visit_date)  as dayscount,
        // --                         quantity_out * quantity_packs  - DATEDIFF(now(),visit_date) *td.quantity * td.frequency as expected_pillcount,
        // --                         tsi.* -- , tdr.*  -- , tvi.* -- td.quantity * td.frequency as pill_count * numberofdays
        // --                         FROM tbl_visit tv, tbl_visit_item tvi, tbl_dose td, tbl_stock_item  tsi, tbl_drug tdr
        // --                         where tv.id = tvi.visit_id
        // --                         and tsi.id = tvi.stock_item_id
        // --                         and patient_id = 6
        // --                         and facility_id = 1
        // --                         and tvi.dose_id = td.id
        // --                         and tdr.id = tsi.drug_id
