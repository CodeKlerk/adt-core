...
class VisitApi extends Controller
{
	...
	$response = DB::table('tbl_stock_item')
               ...
               ->select('drug_id', 'batch_number', 'tbl_unit.name','tbl_visit_item.dose_id', 'expected_pill_count', 'tbl_visit_item.duration', 'quantity_out', 'indication_id', 'expiry_date', 'balance_before')
		       ->get();
	return response()->json($response,200);
    }
